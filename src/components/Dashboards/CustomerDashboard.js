...
const CustomerDashboard = ({ user, onLogout }) => {
        ...
        if (user && (user.id || user._id)) {
          ...
          if (bookingRes.ok) {
            ...
            const formattedBookings = bookingData.map(b => ({
              ...
              // --- FIX: HANDLE FACILITY NAMES ---
              roomName: b.type === 'facility'
                ? (b.facilityName || b.title || 'Event Booking') // Use Facility Name
                : (b.room ? (b.room.name || `Room ${b.room.roomNumber}`) : 'Unknown Room'), // Use Room Name

              // Pass these through for the new BookingHistory component
              type: b.type || 'room',
              title: b.title,
  ...
  const handleBookRoom = (room, checkIn, checkOut) => {
    const newBooking = {
  ...
  // --- 3. HANDLE FACILITY BOOKING FLOW ---
